...
2. 「GougiShogi.config」で合議タイプと将棋ソフト（USIエンジン）を設定します。  
	設定例
	```
	Gougi.Type=多数決合議（3者）
...
3. 将棋所、ShogiGUI等に当プログラムをエンジン登録します。  
	- 将棋所の場合  
	「GougiShogi.jar」をエンジン登録します。  
	- ShogiGUIの場合  
	「GougiShogi.bat」をエンジン登録します。  
...
## 合議アルゴリズム
「GougiShogi.config」で「多数決合議（3者）」「楽観合議」「悲観合議」「楽観合議と悲観合議を交互」「2手前の評価値からの上昇分の楽観合議」「2手前の評価値からの上昇分の悲観合議」「各々の最善手を交換して評価値の合計で判定（2者）」「数手ごとに対局者交代」「序盤・中盤・終盤で対局者交代」「2手前の評価値から一定値以上下降したら対局者交代」「2手前の評価値から一定値以上上昇したら対局者交代」「詰探索エンジンとの合議」「詰探索エンジンとの合議（「脊尾詰」対応版）」「詰探索エンジンとの合議（読み筋の局面も詰探索）」のいずれかを設定してください。  
...
- 序盤・中盤・終盤で対局者交代  
序盤・中盤・終盤で対局者を交代します。  
将棋所等のエンジン設定画面で「G_ChuubanStartPlys（中盤の対局者に交代する手数、例：51）」「G_ShuubanStartPlys（終盤の対局者に交代する手数、例：101）」を指定してください。  
...
「GougiShogi.jar」や「GougiShogi.config」と同じフォルダに「YaneuraOu_sfen変換用.exe」を配置しておいてください。  
...
読み筋の局面で詰みが見つかった場合、通常の探索エンジンへmateinfoコマンド（当プログラム用の独自の拡張コマンド）で通知します。  
  - 読み筋の局面の詰探索に使用する詰探索エンジンの個数を「GougiShogi.config」で指定してください。  
...
（以下、例として技巧と脊尾詰の合議の場合で記載しますが、他のソフトでも同様です）
  ...
  - 将棋所等のエンジン設定画面（脊尾詰のエンジン設定ではなくGougiShogiのエンジン設定）で、詰探索エンジンのタイムアウト（ミリ秒）を「G_MateTimeout」で指定してください。（タイムアウトを短くし過ぎると、脊尾詰が即詰みを逃す場合があります。）
